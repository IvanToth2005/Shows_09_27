...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .card{position:relative;background:#fff;border:1px solid rgba(2,6,23,.08);border-radius:16px;padding:16px;text-decoration:none;color:inherit;box-shadow:0 1px 2px rgba(2,6,23,.04),0 6px 14px rgba(2,6,23,.06);transition:transform .25s cubic-bezier(.2,.8,.2,1),box-shadow .25s,border-color .25s;will-change:transform}
...
<body>
  ...
  <div class="container">
    ...
    <nav class="toolbar" aria-label="Típus szűrő">
      <a class="pill {{ $typeParam==='all' ? 'active' : '' }}" href="{{ route('series.index', ['type'=>'all']) }}">Mind</a>
      @foreach($types as $t)
        <a class="pill {{ (string)$t->id === (string)$typeParam ? 'active' : '' }}" href="{{ route('series.index', ['type'=>$t->id]) }}">{{ $t->name }}</a>
      @endforeach
    </nav>
    ...
    <section class="grid">
      @forelse($series as $s)
        <article class="card">
          <a class="hitarea" href="{{ route('series.show', $s) }}" aria-label="{{ $s->title }} részletei"></a>

          <div class="card-actions">
            <form method="POST" action="{{ route('series.destroy', $s) }}" onsubmit="return confirm('Biztosan törlöd: {{ addslashes($s->title) }}?')" style="margin:0">
              @csrf @method('DELETE')
              <button type="submit" class="icon-btn is-danger" title="Törlés" aria-label="Törlés">
                <i class="fa-solid fa-trash"></i>
              </button>
            </form>
            <a href="{{ route('series.edit', $s) }}" class="icon-btn is-primary" title="Szerkesztés" aria-label="Szerkesztés">
          ...
          <div class="card-body">
            <p class="meta">
              {{ $s->type?->name ?? '—' }}
              @if($s->release_date) • {{ $s->release_date->format('Y') }} @endif
            </p>
            <h3 class="title">{{ $s->title }}</h3>
            <p class="meta">Rendező: {{ $s->director->name }}</p>
            @if($s->seasons) <p class="meta">Évadok: {{ $s->seasons }}</p> @endif
            @if($s->length)  <p class="meta">Epizód-hossz: {{ $s->length }} perc</p> @endif
          </div>
        </article>
      @empty
        <p>Nincs találat.</p>
    ...
    <div style="margin-top:14px">{{ $series->links() }}</div>
  </div>
</body>
</html>
